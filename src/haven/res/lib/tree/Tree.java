...
public class Tree extends Sprite {
    ...
    public static Collection<Pair<Integer, RenderTree.Node>> lsparts(Resource res, int matsel) {
    ...
    public static RenderTree.Node[][] mkparts(Resource res, int matsel, int fl) {
	...
	final Optional<MCache.Grid> grid = owner.glob.map.getgridto(tc);
	final Random rnd;
	if(grid.isPresent()) {
	    tc = tc.sub(grid.get().ul);
	    rnd = new Random(grid.get().id);
	} else {
	    rnd = new Random();
	}
