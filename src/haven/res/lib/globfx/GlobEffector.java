...
public class GlobEffector extends Drawable {
    ...
    static final Map<Glob, Reference<GlobEffector>> cur = new WeakHashMap<>();
	...
	data.values().removeIf(d -> d.tick(dt));
	synchronized(cur) {
	...
	synchronized(cur) {
	    ...
	    if(ret == null) {
		final Gob hgob = new Gob(glob, Coord2d.z) {
			public Coord3f getc() {
			    return(Coord3f.o);
			}
		...
		cur.put(glob, new WeakReference<>(ret = ne));
	    }
	}
	if(add != null)
	    glob.oc.ladd(add);
