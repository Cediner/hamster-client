...
public class Coord implements Comparable<Coord>, java.io.Serializable {
    ...
    public static Coord of(float x, float y) {
	return (new Coord(x, y));
    }
	...
	return of((int) Math.round(x / dx), (int) Math.round(y / dy));
    }
	...
	if (m1 != 0) {
	    return Optional.of(of((int) (Double.isFinite(m1) ? (b2 - b1) / m1 : this.x), b2));
	} else {
	...
	if (Double.isFinite(m)) {
	    return Optional.of(of(x, (int) (m * x + b)));
	} else {
	    return Optional.empty();
	}
    }
	...
	return of(Math.abs(x), Math.abs(y));
    }
	...
	return of(Math.min(x, d.x), Math.min(y, d.y));
    }

    public Coord max(final Coord d) {
	return of(Math.max(x, d.x), Math.max(y, d.y));
    }
