...
public class FastText {
	...
	g.dispose();
	final var fbuf = TexI.mkbuf(new Coord(buf.getWidth(), buf.getHeight()*2));
	final var outlines = Utils.outline2(buf, Color.BLACK);
	final var fg = fbuf.createGraphics();
	fg.drawImage(buf, 0, 0, null);
	fg.drawImage(outlines, 0, buf.getHeight(), null);
	ct = new TexI(fbuf);
    }
    ...
    public static void asprint(GOut g, Coord c, double ax, double ay, String text) {
	Coord lc = c.add(g.tx);
	...
	if(lc.isect(g.parent().ul, g.parent().sz())) {
		...
		short tx1 = (short) (((sx[cc] * 65535) + (ct.tdim.x / 2)) / ct.tdim.x),
			tx2 = (short) ((((sx[cc] + w) * 65535) + (ct.tdim.x / 2)) / ct.tdim.x);
		short ty1 = (short)((h * 65535) / ct.tdim.y), ty2 = (short) ((h * 2 * 65535) / ct.tdim.y);
		data[so + 0] = x1;
		data[so + 1] = y1;
		data[so + 2] = tx1;
		data[so + 3] = ty1;
    ...
    public static void asprintf(GOut g, Coord c, double ax, double ay, String fmt, Object... args) {
	asprint(g, c, ax, ay, String.format(fmt, args));
    }
