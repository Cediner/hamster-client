...
public class ChatUtils {
    ...
    private static Object parseArg(final String argstr, final int[] ref) {
        ...
        return switch (type) {
            case "string" -> nextToken(argstr, ref);
            case "number" -> Long.parseLong(nextToken(argstr, ref));
            case "real" -> Double.parseDouble(nextToken(argstr, ref));
            case "coord" -> new Coord(Integer.parseInt(nextToken(argstr, ref)),
                    Integer.parseInt(nextToken(argstr, ref)));
            case "coord3f" -> new Coord3f(Float.parseFloat(nextToken(argstr, ref)),
                    Float.parseFloat(nextToken(argstr, ref)),
                    Float.parseFloat(nextToken(argstr, ref)));
            case "coord2d" -> new Coord2d(Double.parseDouble(nextToken(argstr, ref)),
                    Double.parseDouble(nextToken(argstr, ref)));
            default -> throw new RuntimeException("Invalid dispatch message - " + argstr);
        };
    }
