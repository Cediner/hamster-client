...
public class DmgFilter implements Filter {
    public static final String TAG = "dmg";
    public static final String pattern = "dmg\\s*(?<op>(<|>|=|(<=)|(>=)))\\s*(?<arg>([0-9]+(\\.[0-9]+)?))";
    public static Optional<Filter> make(final Matcher match) {
	final var op = NumberOp.parse(match.group("op"));
	return Optional.of(new DmgFilter(op, Float.parseFloat(match.group("arg"))));
    }

    private final NumberOp op;
    private final float value;

    public DmgFilter(final NumberOp op, final float value) {
	this.op = op;
	this.value = value;
    }
	...
	final var dmgo = item.getinfo(Damage.class);
