...
public class KnarrSprite extends VarSprite implements SailSprite {
    static final FastMesh proj = Resource.remote().loadwait("gfx/terobjs/vehicle/knarr").layer(FastMesh.MeshRes.class, 16).m;
    static final Material base = Resource.remote().loadwait("gfx/terobjs/vehicle/knarr").layer(Material.Res.class, 16).get();

    public KnarrSprite(final Owner owner, final Resource res, final Message sdt) {
	super(owner, res, sdt);
    }
