...
/**
 * TODO: REDO, valid movements are N, NE, NW, W, SW, S, SE, E -> Using diagonalDistance
 */
public abstract class Pathfinder {
    static final Coord[][] dirs = new Coord[1][4];
    private static final Hitbox.Rectangular plhb;

    static {
        plhb = (Hitbox.Rectangular)Hitbox.hbfor("gfx/borka/body");
    ...
    private boolean areWeBoating() {
        ...
        if (me != null && me.isHeldBySomething()) {
            final Gob holder = ui.sess.glob.oc.getgob(me.whoIsHoldingMe());
            return holder != null && holder.hasTag(Tag.WATER_VEHICLE);
        } else {
            return false;
        }
    }
     ...
     * Currently only works when going in non-angled directions
     * TODO: account for angled moves if we ever go to diagonal movements
     */
    private boolean hitGob(final Coord mc) {
        return ui.sess.glob.gobhitmap.checkHit(plhb, mc, 0);
    }
