...
public class SessionDetails {
    ...
    public void attachInventory(final Inventory inv, final InventoryType type) {
        synchronized (invs) {
            switch (type) {
                case MAIN -> maininv = new WeakReference<>(inv);
                case BELT -> beltinv = new WeakReference<>(inv);
                case STUDY -> studyinv = new WeakReference<>(inv);
                case SUPPLEMENTAL -> invs.add(new WeakReference<>(inv));
            }
        }
    }
    ...
    public void removeInventory(final Inventory inv, final InventoryType type) {
        synchronized (invs) {
            ...
            switch (type) {
                case MAIN -> maininv = null;
                case BELT -> beltinv = null;
                case STUDY -> studyinv = null;
                case SUPPLEMENTAL -> invs.removeIf(i -> i.get() == null || i.get() == inv);
            }
        }
    }
